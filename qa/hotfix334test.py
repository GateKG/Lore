...
check("lore.py APP_VERSION 3.37", 'APP_VERSION = "3.37"' in SRC)
check("ui.html version:'3.37' x2, no 3.36 stamp left",
      USRC.count("version:'3.37'") == 2 and "version:'3.36'" not in USRC)
check("version.txt 3.37", "filevers=(3, 37, 0, 0)" in VT
      and "prodvers=(3, 37, 0, 0)" in VT and VT.count("'3.37.0.0'") == 2
      and "3, 33" not in VT and "3.33" not in VT)
check("installer.iss 3.37", "AppVersion=3.37\n" in ISS.replace("\r\n", "\n")
